 ...
 */
class PetFileRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function ajax_list(array $get, array $userData){
        ...
        $columns = array(
            array("DATE_FORMAT(pt.`created_at`, '%m/%d/%Y')", "DATE_FORMAT(pt.`created_at`, '%m/%d/%Y')", 'petFileDate'),
            array('pt.`description`', 'pt.`description`'),
            array('pt.`id`', 'pt.`id`'),
            array('pt.`pet_id`', "pt.`pet_id`"),
            array('pt.`parsed_description`', "pt.`parsed_description`"),
        ...
        */
        ...
        $url = $get['url'];
        $formUrl = $url . 'pet_file/ajax_form';  
        $hasUpdate = $userData['type'] == 'Super Admin'  || in_array('Shelter Admission Details Pet Files Update', $userData['accesses']) ? true : false;
        ...
        foreach($result as $row) {
            ...
            $action ='';
                ...
                $values = array(
                    $row['petFileDate'],
                    "<a href='".$url."uploads/file/".$row['parsed_description']."' target='_blank'>".$row['description']."</a>",
                    $action
                );
