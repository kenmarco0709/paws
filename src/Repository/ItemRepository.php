...
class ItemRepository extends \Doctrine\ORM\EntityRepository
{
    public function validate($form) {
        ...
        if($action !== 'd') {
            ...
            $itemExist = $this->getEntityManager()->getRepository(ItemEntity::class)
                ->createQueryBuilder('c')
                ->where('c.id != :id')
                ->andWhere('c.description = :description')
                ->andWhere('c.isDeleted = :is_deleted')
                ->andWhere('c.itemCategory = :itemCategory')
            ...
            if($action != 'u' && $itemExist){
                $errors[] = 'Item already exist.';
            }

            if(empty($form['item_category'])){
                $errors[] = 'Category should not be blank.';
            }
    ...
    public function ajax_list(array $get){
        ...
        $columns = array(
            array('ci.`description`', 'ci.`description`', 'categoryItem'),
            array('c.`code`', 'c.`code`'),
        ...
        $select = "SELECT";
        $from = "FROM `item` c";
        $joins = " LEFT JOIN `item_category` ci ON ci.`id` = c.`item_category_id` 
        ";
