...
class ClientRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function validate($client_form) {
        ...
        if($action !== 'd') {
            ...
            // if(empty($client_form['email'])) {
            //     $errors[] = 'Email should not be blank.';
            // }

            // if(empty($client_form['contact_no'])) {
            //     $errors[] = 'Contact No. should not be blank.';
            // }

            
            // if(empty($client_form['address'])) {
            //     $errors[] = 'Address should not be blank.';
            // }
        }
    ...
    public function ajax_list(array $get, $userData){
        ...
        foreach($result as $row) {
            ...
            $details = $hasDetails ? "<a class='action-button-style ' href='$detailsUrl/$id'>". $row['fullName']."</a>" : "";
    ...
    public function getYearlyAnalytics(array $userData){
        ...
        $query = $this->getEntityManager()->getConnection()->prepare("
                ...
                LEFT JOIN(
                    ...
                    ".$andWhere."
                    AND i.`status` = 0
                    GROUP BY DATE_FORMAT(i.`created_at`, '%M') 
                ) i ON DATE_FORMAT(i.createdAt, '%M') = DATE_FORMAT(cte.`dt`, '%M')
        ");
