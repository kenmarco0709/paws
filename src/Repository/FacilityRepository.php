...
class FacilityRepository extends \Doctrine\ORM\EntityRepository
{
    public function validate($form) {
        ...
        if($action !== 'd') {
            ...
            $facilityExist = $this->getEntityManager()->getRepository(FacilityEntity::class)
                ->createQueryBuilder('c')
                ->where('c.id != :id')
                ->andWhere('c.description = :description')
                ->andWhere('c.isDeleted = :is_deleted')
                ->andWhere('c.species = :facilitySpecies')
                ->andWhere('c.branch = :branch')
                ->setParameters(array(
                    'id' => $form['id'],
                    'description' => $form['description'],
                    'is_deleted' => false,
                    'facilitySpecies' => $form['species'],
                    'branch' => base64_decode($form['branch'])

                ))
                ->getQuery()->getResult();
            
            if($action != 'u' && $facilityExist){
                $errors[] = 'Facility already exist.';
            }

            if(empty($form['species'])){
                $errors[] = 'Species should not be blank.';
            }
    ...
    public function ajax_list(array $get){
        ...
        $columns = array(
            array('s.`description`', 's.`description`', 'species'),
            array('c.`code`', 'c.`code`'),
            array('c.`description`', 'c.`description`'),
            array('c.`capacity`', 'c.`capacity`'),
            array('c.`id`', "c.`id`")
        );
        $asColumns = array();

        $select = "SELECT";
        $from = "FROM `facility` c";
        $joins = " LEFT JOIN `species` s ON s.`id` = c.`species_id` 
        ";
