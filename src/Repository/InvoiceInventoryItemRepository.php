...
/**
 * InvoiceInventoryItemRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class InvoiceInventoryItemRepository extends \Doctrine\ORM\EntityRepository
{

    public function inventory_item_ajax_list(array $get){

        $columns = array(
            array('DATE_FORMAT(iii.`created_at`, "%m/%d/%Y")', 'DATE_FORMAT(iii.`created_at`, "%m/%d/%Y")', 'date'),
            array('iii.`id`', 'iii.`id`'),
            array('iii.`quantity`', 'iii.`quantity`'),
        );
        $asColumns = array();

        $select = "SELECT";
        $from = "FROM `invoice_inventory_item` iii";
        $joins = "";
        $sqlWhere = "WHERE iii.`is_deleted` = 0";
        $groupBy = "";
        $orderBy = "";
        $limit = "";
        $stmtParams = array();

        $sqlWhere.= " AND iii.`inventory_item_id` = :inventoryItemId";
