...
use App\Entity\PaymentTypeAccessEntity;
...
class PaymentTypeController extends AbstractController
{
    ...
    public function ajax_list(Request $request, AuthService $authService) {
        ...
        if($authService->isLoggedIn()) {
            ...
            $hasUpdate = $authService->getUser()->getType() == 'Super Admin'  || $authService->isUserHasAccesses(array('CMS Payment Type Update')) ? true : false;
            $data = $this->getDoctrine()->getManager()->getRepository(PaymentTypeEntity::class)->ajax_list($get, $this->get('session')->get('userData'));
            ...
            foreach($data['results'] as $row) {
                ...
                if($hasUpdate) {
                    $url = $this->generateUrl('payment_type_form', array(
                        'action' => 'u',
                        'id' => $id
                    ));
    ...
    public function formAction($action, $id, Request $request, AuthService $authService) {
        ...
        return $this->render('PaymentType/form.html.twig', array(
            'javascripts' => array('/js/payment_type/form.js'),
