...
class AuthController extends AbstractController
{
    ...
    public function index(Request $request, AuthService $authService)
    {
       ...
       if($authService->isLoggedIn()) return $this->redirect($this->generateUrl('schedule_index'), 302);
                    ...
                    if(!$user){
                        ...
                        if($user->getPassword() == crypt(md5($authForm['password']), $user->getPassword())){
                            ...
                            $userData = array(
                                ...
                                'branchId' => $user->getBranch() ? $user->getBranch()->getId() : '', 
                            ...
                            $url = $this->generateUrl('schedule_index');

                            $response = new RedirectResponse($url);
                            $response->headers->setCookie(new Cookie('userId', $user->getIdEncoded(), (time() + (86400))));
                            $response->headers->setCookie(new Cookie('username', $user->getUsername(), (time() + (86400))));
